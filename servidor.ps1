...
while (-not $started -and $port -le 5185) {
    try {
...
if (-not $started) {
    Write-Host "Error: No se pudo iniciar el servidor en los puertos 5173-5185." -ForegroundColor Red
    exit 1
}
...
# Abrir en Google Chrome si esta instalado, o en el navegador predeterminado
$chromePaths = @(
...
foreach ($cPath in $chromePaths) {
    if (Test-Path $cPath) {
        Start-Process $cPath "http://localhost:$port"
        $chromeLaunched = $true
        break
    }
}
...
while ($listener.IsListening) {
    try {
        ...
        # Asegurar separadores de ruta en Windows
        ...
        # Evitar caidas del servidor por desconexiones del cliente
    }
}
